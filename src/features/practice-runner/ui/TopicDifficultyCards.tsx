  ...
  return (
    <div className="grid [grid-auto-rows:1fr] gap-6 md:grid-cols-3">
      {difficulties.map((difficulty) => (
        <article
          key={difficulty.id}
          className="flex h-full flex-col rounded-3xl border-slate-200 bg-white p-6 shadow-[0_20px_60px_rgba(15,23,42,0.08)] transition duration-200 hover:-translate-y-1 hover:shadow-[0_24px_70px_rgba(15,23,42,0.14)]"
        >
          <div className="flex flex-1 flex-col">
            <div className="mb-3 flex items-start justify-between gap-3">
              <h2 className="text-2xl leading-tight font-semibold" style={{ color: "#0f172a" }}>
                {difficulty.title}
              </h2>
            ...
            <p className="text-base leading-6" style={{ color: "#475569" }}>
              {difficulty.description}
            </p>

            <div className="mt-3 text-sm font-medium text-[#13b2f6]">
              {t("pointsReward", { points: difficulty.points })}
            </div>

            <div className="mt-auto pt-6">
              <Link href={buildHref(difficulty.id)}>
